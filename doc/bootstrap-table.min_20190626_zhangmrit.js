...
(function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showSearch) {
        	html.push(sprintf('<button class="btn' +
                ...
                sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.search),
                '</button>');
        }
		...
		if (this.options.showSearch) {
			this.$toolbar.find('button[name="showSearch"]')
            .off('click').on('click', function () {
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (this.options.showPageGo) {
            var that = this,
                ...
                $pageGo = $pageGroup.find('li.pageGo');

            if (!$pageGo.length) {
                $pageGo = $([
                    '<li class="pageGo">',
                        sprintf('<input type="text" class="form-control" value="%s">', this.options.pageNumber),
                        '<button class="btn' +
                            sprintf(' btn-%s', this.options.buttonsClass) +
                            sprintf(' btn-%s', this.options.iconSize) +
                            '" title="' + this.options.formatPageGo() + '" ' +
                            ' type="button">'+this.options.formatPageGo(),
                        '</button>',
                    '</li>'].join('')).appendTo($pageGroup);

                $pageGo.find('button').click(function () {
                    var page=parseInt($pageGo.find('input').val())||1;
    ...
    BootstrapTable.prototype.updateSelected = function () {
        ...
        var fixedColumn = $('.left-fixed-table-columns input[name=btSelectItem]');
        if(fixedColumn.length > 0) {
            checkAll = this.$selectItem.filter(':enabled').length &&
            this.$selectItem.filter(':enabled').length ===
            fixedColumn.filter(':enabled').filter(':checked').length;
        }
